...
VERSION = "v0.0.1"
GITHUB_USER = "jobe95"  # ← ändra detta
...
TASKS = [
    {
        "id": "t1",
        "title": "Lås bokföringsperiod januari",
        "deadline": "2026-03-22",
        "cat": "Bokföring",
    },
    {
        "id": "t4",
        "title": "Lås bokföringsperiod februari",
        "deadline": "2026-03-31",
        "cat": "Bokföring",
    },
    {
        "id": "t5",
        "title": "Lås bokföringsperiod mars",
        "deadline": "2026-04-30",
        "cat": "Bokföring",
    },
    {
        "id": "t6",
        "title": "Arbetsgivardeklaration mars",
        "deadline": "2026-04-12",
        "cat": "Lön",
    },
    {
        "id": "t7",
        "title": "Betala preliminärskatt april",
        "deadline": "2026-04-12",
        "cat": "Skatt",
    },
    {
        "id": "t2",
        "title": "Bokför alla transaktioner Q1",
        "deadline": "2026-04-30",
        "cat": "Bokföring",
    },
    {
    ...
    {
        "id": "t8",
        "title": "Momsdeklaration Q2",
        "deadline": "2026-08-17",
        "cat": "Moms",
    },
    {
        "id": "t9",
        "title": "Momsdeklaration Q3",
        "deadline": "2026-11-17",
        "cat": "Moms",
    },
    {
...
def check_for_update():
    ...
    try:
        req = urllib.request.Request(
            GITHUB_API, headers={"User-Agent": "BearFieldKalender"}
        )
...
class BearFieldApp(rumps.App):
    ...
    def rebuild_menu(self):
        ...
        urgent = [
            t
            for t in TASKS
            if not done.get(t["id"]) and 0 <= days_until(t["deadline"]) <= 7
        ]
        ...
        upcoming = [
            t for t in TASKS if not done.get(t["id"]) and days_until(t["deadline"]) > 7
        ]
        if upcoming:
        ...
        items.append(
            rumps.MenuItem("🔄  Sök uppdateringar", callback=self.check_update)
        )
        items.append(rumps.separator)
    ...
    def test_notification(self, _):
        ...
        upcoming = [
            t for t in TASKS if not done.get(t["id"]) and days_until(t["deadline"]) >= 0
        ]
        if not upcoming:
            rumps.notification(
                "BearField IT",
                "Inga kommande deadlines",
                "Alla uppgifter är avklarade!",
            )
            return
        ...
        def _check():
            ...
            if not result:
                rumps.notification(
                    "BearField IT",
                    "Redan uppdaterad",
                    f"Du kör senaste versionen ({VERSION})",
                )
                return
            latest, notes = result
